...
- **MCP server magic** that makes Claude Desktop sing
...
curl -fsSL https://jarule.dev/setup | sh -s -- --dotfiles-only

# Just want the AI rules? (Skip the dotfiles)
curl -fsSL https://jarule.dev/setup | sh -s -- --rules-only

# Or if you're the cautious type
